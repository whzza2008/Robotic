...
//initiate user button
  ...
  /* -----------------------------------------------------------------------
    ...
    CC1 update rate = TIM2 counter clock / CCR1_Val = 9.154 Hz
 ...
 /* TIM Configuration */
  ...
  while (1){ 
			...
			if (UBPressed==1) { 
				...
				factor = (factor+1)%2;
				if (factor == 1){
				  TIM_PrescalerConfig(TIM2, ((uint16_t) ((SystemCoreClock / 2) / (54618*2)) - 1), TIM_PSCReloadMode_Immediate);
					/* make the two LEDs blink alternatively instead of doing together. */
					STM_EVAL_LEDToggle(LED3);
				}else{
					TIM_PrescalerConfig(TIM2, PrescalerValue, TIM_PSCReloadMode_Immediate);
				}
				/* restart interrupt on TIM2 */
				TIM_ITConfig(TIM2, TIM_IT_CC1, ENABLE);
				TIM_Cmd(TIM2, ENABLE);
